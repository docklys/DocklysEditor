...
public partial class MainWindow
{
    ...
    private string GetCurrentThemeImagesDir()
    {
        if (_currentIndex >= 0 && _currentIndex < _catalog.Count)
            return Path.Combine(ThemeLibraryDir, "Images", _catalog[_currentIndex].Id);
        return Path.Combine(ThemeLibraryDir, "Images", "DockCaptures");
    }
    ...
    private async void CaptureToWebP_Click(object? sender, RoutedEventArgs e)
    {
        ...
        var imagesDir = GetCurrentThemeImagesDir();
    ...
    {
        var dir = GetCurrentThemeImagesDir();
        Directory.CreateDirectory(dir);
        OpenFolder(dir);
    }
