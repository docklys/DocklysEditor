...
public partial class MainWindow
{
    ...
    {
        if (_catalog.Count == 0) { await ShowMessageDialog("GitHub", "No theme selected."); return; }

        var entry = _catalog[_currentIndex];
        var repoName = "Docklys.Theme." + entry.Name.Replace(" ", "");
        ...
        var suggestedDesc = isUpdate ? "Update theme files" : $"A Docklys theme: {entry.Name}";

        var spec = await PromptGitHubSpec(repoName, suggestedDesc, isUpdate);
        if (spec == null) return;

        await RunGitHubFlow(ThemeLibraryDir, spec, isUpdate);
    }
    ...
    private async Task RunGitHubFlow(string folder, GitHubSpec spec, bool isUpdate)
    {
        ...
        try
        {
            // Create a temp folder with just the selected theme's .dockly file + README
            var tempDir = Path.Combine(Path.GetTempPath(), "docklys-theme-" + Guid.NewGuid().ToString("N")[..8]);
            Directory.CreateDirectory(tempDir);

            var srcFile = _catalog[_currentIndex].FilePath;
            if (File.Exists(srcFile))
                File.Copy(srcFile, Path.Combine(tempDir, Path.GetFileName(srcFile)));
            ...
            if (!gitInited)
            {
                var (initOk, initLog) = await RunProcessAsync("git", "init", tempDir);
                if (!initOk) { await ShowMessageDialog("git init failed", initLog); return; }
            }

            if (isUpdate)
            {
                await RunProcessAsync("git", "add .", tempDir);
                await RunProcessAsync("git", $"commit -m \"{spec.Description}\"", tempDir);

                if (!gitInited)
                {
                    var user = GetGitHubUser();
                    await RunProcessAsync("git", $"remote add origin https://github.com/{user}/{spec.RepoName}.git", tempDir);
                    var (okPush, logPush) = await RunProcessAsync("git", "push -u origin main", tempDir);
                ...
                {
                    var (okPush, logPush) = await RunProcessAsync("git", "push", tempDir);
                    if (okPush) await ShowMessageDialog("Repository Updated", $"✓ '{spec.RepoName}' updated on GitHub.");
                    else await ShowMessageDialog("Update Failed", $"git push failed:\n\n{logPush}");
                }
            ...
            await RunProcessAsync("git", "add .", tempDir);
            await RunProcessAsync("git", $"commit -m \"Initial commit: {spec.RepoName}\"", tempDir);
            ...
            if (ok)
                await ShowMessageDialog("Repository Created",
                    $"✓ '{spec.RepoName}' pushed to GitHub with the .dockly file.");
            else
