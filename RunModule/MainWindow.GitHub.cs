...
public partial class MainWindow
{
    ...
    {
        if (_catalog.Count == 0) { await ShowMessageDialog("GitHub", "No module selected."); return; }

        var entry = _catalog[Math.Clamp(_currentIndex, 0, _catalog.Count - 1)];
        var folder = Path.GetDirectoryName(entry.CsprojPath);
        if (folder == null || !Directory.Exists(folder))
        {
            await ShowMessageDialog("GitHub", "Module source folder not found.");
            return;
        }

        var repoName = "Docklys." + entry.FolderName;
        ...
        var suggestedDesc = isUpdate ? "Update module files" : $"A Docklys module: {entry.FolderName}";
        ...
        try
        {
            ...
            if (ok)
                await ShowMessageDialog("Repository Created",
                    ...
                    $"Visit: https://github.com/{GetGitHubUser()}/{spec.RepoName}");
            else
    ...
    private async Task<bool> EnsureGhAuth()
    {
        ...
        loginBtn.Click += async (_, _) => {
            ...
            try {
                var ghPath = ResolveGhPath();
                // We use UseShellExecute = true to ensure the browser can be launched by gh.
                // We run via powershell to try and pipe an Enter key for the common "Press Enter to open browser" prompt.
    ...
    private async Task<GitHubSpec?> PromptGitHubSpec(string suggestedName, string suggestedDesc, bool isUpdate)
    {
        ...
        var nameBox = new TextBox { Width = 340, Text = suggestedName, Watermark = "e.g. Docklys.MyModule", IsReadOnly = isUpdate };
        var descBox = new TextBox { Width = 340, Text = suggestedDesc, Watermark = "Short description" };
        var publicCheck = new CheckBox { Content = "Public repository", IsChecked = true, Foreground = Brushes.White, IsVisible = !isUpdate };

        var ok = new Button { Content = isUpdate ? "Commit & Push" : "Create & Push", IsDefault = true, Padding = new Thickness(16, 4) };
