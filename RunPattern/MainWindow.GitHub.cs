...
public partial class MainWindow
{
    ...
    {
        if (_catalog.Count == 0) { await ShowMessageDialog("GitHub", "No pattern selected."); return; }

        var entry = _catalog[Math.Clamp(_currentIndex, 0, _catalog.Count - 1)];
        var folder = Path.GetDirectoryName(entry.CsprojPath);
        if (folder == null || !Directory.Exists(folder))
        {
            await ShowMessageDialog("GitHub", "Pattern source folder not found.");
            return;
        }

        var repoName = "Docklys.Pattern." + entry.FolderName;
        ...
        var suggestedDesc = isUpdate ? "Update pattern files" : $"A Docklys pattern: {entry.FolderName}";
