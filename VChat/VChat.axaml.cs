...
namespace VChat
{
    public partial class VChat : UserControl, IModule, IResizable, IInteractionFreezable
    {
        ...
        private const string VChatUrl = "https://v.qwqc.dedyn.io/";
        ...
        private void SyncWebViewHwndToVisualBounds()
        {
            ...
            Console.WriteLine($"[VChat] SyncWebViewHwndToVisualBounds: localBounds={local.Width}x{local.Height}");
        ...
        private bool ForceWebView2RepositioningWithSize(int physicalOffsetX, int w, int h, double? zoom = null)
         {
             ...
             try
             {
                 ...
                 var rect = new System.Drawing.Rectangle(physicalOffsetX, 0, w, h);
                Console.WriteLine($"[VChat] ForceWebView2RepositioningWithSize rect={{X={rect.X},Y={rect.Y},W={rect.Width},H={rect.Height}}} zoom={zoom}");
                 _controllerBoundsPropCache    ??= controllerType.GetProperty("Bounds");
                 _controllerBoundsPropCache?.SetValue(controller, rect);
