...
// as the main window — a direct port of RunPattern's dialog helpers so the
// plugin editor's Rename / New / Delete flows look and feel identical.
public partial class MainWindow
{
    private sealed record NewPluginSpec(string Name, int TemplateIndex);
    ...
    private async Task<NewPluginSpec?> PromptForNewPluginSpec(string pluginsDir)
    {
        var tcs = new TaskCompletionSource<NewPluginSpec?>();
        ...
        var nameBox = new TextBox { Width = 320, Watermark = "e.g. MyPlugin" };
        ...
        var window = new Window
        {
            ...
            Content = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    templateBox,
                    new TextBlock { Text = "Plugin name", Foreground = Brushes.White, Margin = new Thickness(0, 12, 0, 0) },
                    nameBox,
                    hint,
                    new StackPanel
                    {
        ...
        okBtn.Click += async (_, _) =>
        {
            ...
            var (ok, reason) = ValidatePluginName(raw, pluginsDir);
            if (!ok) { await ShowMessageDialog("Invalid name", reason!); nameBox.Focus(); return; }
            tcs.TrySetResult(new NewPluginSpec(raw, Math.Max(0, templateBox.SelectedIndex)));
            window.Close();
        };
    ...
    private async Task<string?> PromptForPluginName(string? initial = null, string title = "Rename Plugin")
    {
        var tcs = new TaskCompletionSource<string?>();

        var textbox = new TextBox { Width = 320, Watermark = "e.g. MyPlugin", Text = initial ?? "" };
        var hint = new TextBlock
        {
            Text = "Letters, digits, and underscores. Must start with a letter or underscore.",
            FontSize = 11,
        ...
        var window = new Window
        {
            ...
            Content = new StackPanel
            {
                ...
                Children =
                {
                    new TextBlock { Text = "Plugin name", Foreground = Brushes.White },
                    textbox,
                    hint,
                    new StackPanel
                    {
