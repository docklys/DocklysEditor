...
// validation, RunPlugin.csproj <ProjectReference> patching, best-effort
// DefaultModule.sln edits, dotnet build, and catalog refresh. Mirrors the
// equivalents in RunPattern.
public partial class MainWindow
{
    private static readonly Regex PluginIdentifierRegex =
        new(@"^[A-Za-z_][A-Za-z0-9_]*$", RegexOptions.Compiled);

    // Folder names we never let a plugin take.
    private static readonly string[] ReservedPluginNames =
        { "RunPlugin", "Docklys.ModuleContracts" };
    ...
    private void ReloadCatalogAndSelect(string folderName)
    {
        ...
        ShowPluginAtIndex(idx);
    }

    private static (bool ok, string? reason) ValidatePluginName(
        string name, string pluginsDir, string? allowExistingFolder = null)
    {
        if (string.IsNullOrWhiteSpace(name))
            return (false, "Plugin name cannot be empty.");
        if (!PluginIdentifierRegex.IsMatch(name))
            return (false,
                "Plugin name must be a valid identifier — start with a letter or " +
                "underscore, then letters/digits/underscores. No spaces or special characters.");
        if (ReservedPluginNames.Any(r => string.Equals(r, name, StringComparison.OrdinalIgnoreCase)))
        ...
        if (Directory.Exists(pluginsDir))
        {
            foreach (var folder in Directory.EnumerateDirectories(pluginsDir))
            {
                ...
                if (string.Equals(folderName, name, StringComparison.OrdinalIgnoreCase))
                    return (false, $"A plugin folder named '{folderName}' already exists. Pick a different name.");
            }
        }
    ...
    // RunPlugin.csproj only references the bundled plugins; user-scaffolded ones
    // aren't listed, so these patches are best-effort no-ops when absent.
    private static string? TryRetargetProjectReferenceInRunPlugin(string solutionDir, string oldName, string newName)
    {
        var csproj = Path.Combine(solutionDir, "RunPlugin", "RunPlugin.csproj");
        if (!File.Exists(csproj)) return null;

        var text = File.ReadAllText(csproj);
        var oldInclude = $"..\\Plugins\\{oldName}\\{oldName}.csproj";
        var newInclude = $"..\\Plugins\\{newName}\\{newName}.csproj";
    ...
    private static void StripProjectReferenceFromRunPlugin(string solutionDir, string folderName)
    {
        var csproj = Path.Combine(solutionDir, "RunPlugin", "RunPlugin.csproj");
        if (!File.Exists(csproj)) return;

        var text = File.ReadAllText(csproj);
        var marker = $"<ProjectReference Include=\"..\\Plugins\\{folderName}\\{folderName}.csproj\" />";
