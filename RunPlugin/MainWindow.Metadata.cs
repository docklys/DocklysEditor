...
public partial class MainWindow
{
    private sealed class DocklysMeta
    {
        ...
        [JsonPropertyName("type")]        public string Type        { get; set; } = "plugin";
    ...
    private void RefreshMetaForCurrentPlugin()
    {
    ...
    private static void CopyMetaToOutput(string folder)
    {
        ...
        var outDir = Path.Combine(editorRoot, "OutputPluginDLL");
    ...
    private async Task ShowInfoDialogAsync(string folder, DocklysMeta meta)
    {
        var descBox = new TextBox
        {
            ...
            Watermark      = "Describe your plugin for the marketplace…",
            AcceptsReturn  = true,
            MinHeight      = 80,
            MaxHeight      = 180,
        ...
        void Row(string label, string value) => fields.Children.Add(new Grid
        {
        ...
        var w = new Window
        {
            Title = $"Plugin Info — {meta.Name}", Width = 460, Height = 380,
            MinHeight = 260, CanResize = true,
            WindowStartupLocation = WindowStartupLocation.CenterOwner, Content = dp,
        };
        StyleDialog(w);
        ...
        syncBtn.Click += async (_, _) =>
        {
            ...
            try
            {
                ...
                if (remOk && !string.IsNullOrWhiteSpace(rem))
                {
                    ...
                    {
                        var gh = ResolveGhPath();
                        var (dOk, d) = await RunProcessAsync(gh,
                            $"api repos/{owner}/{repo} --jq .description", AppContext.BaseDirectory);
                        if (dOk && !string.IsNullOrEmpty(d.Trim()) && d.Trim() != "null")
                        {
                            descBox.Text = d.Trim(); meta.GithubRepo = $"{owner}/{repo}";
                            syncBtn.Content = "✓ Synced"; await Task.Delay(1200);
                        }
    ...
    {
        if (_catalog.Count == 0) { await ShowMessageDialog("Images", "No plugin loaded."); return; }
        var entry     = _catalog[Math.Clamp(_currentIndex, 0, _catalog.Count - 1)];
        var folder    = Path.GetDirectoryName(entry.CsprojPath)!;
